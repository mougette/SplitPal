...
class Split extends Component {
  ...
  for(let i = start; i < newMax; i++){
      ...
      let owedArray = [this.email, this.friend.Email]
           ...
           'price': 0-parseFloat(this.props.route.params[2].items[i-start][1].replace(/\$|,/g, '')),
           'users' : owedArray});
           this.setState({
             inputData: dataArray
      });
  ...
  addTextInput = (index) => {
    ...
    <TextInput style={styles.input}
      ...
      <TextInput style={styles.input2}
            ...
            defaultValue={this.state.inputData[index] == undefined ? "" : Math.abs(this.state.inputData[index].price).toString()}/>
      </View>
      );
